...
<body style="background: radial-gradient(circle at 20% 30%, rgba(26, 42, 68, 0.7) 20%, transparent 50%), radial-gradient(circle at 80% 20%, rgba(168, 85, 247, 0.5) 25%, transparent 50%), radial-gradient(circle at 50% 80%, rgba(255, 51, 51, 0.5) 20%, transparent 50%), radial-gradient(circle at 30% 70%, rgba(59, 130, 246, 0.6) 20%, transparent 50%), linear-gradient(to bottom, #1a2a44, #3b82f6); color: white; font-family: Arial; margin: 0; padding: 20px; height: 100vh; width: 100vw; overflow: hidden; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; position: relative; max-width: 414px; margin: 0 auto;">
  <div id="topContainer" style="display: flex; justify-content: center; align-items: center; gap: 20px; margin: 20px 10px 20px 10px;">
  ...
  <div id="toggleContainer" style="display: flex; justify-content: center; gap: 20px; margin-bottom: 10px;">
    <div id="pulseToggle" class="toggle-button toggled" style="width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 24px; cursor: pointer; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.5); border: none;">💓</div>
    <div id="waveToggle" class="toggle-button" style="width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 24px; cursor: pointer; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.5); border: none;">〰️</div>
  </div>
  ...
  <div id="sliderTrack" style="width: 120px; height: 80%; max-height: 600px; background-color: #1e1b4b; border-radius: 60px; position: relative; margin: 20px auto; overflow: hidden; display: flex; flex-direction: column; justify-content: space-between; align-items: center; padding: 10px 0; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.5);">
    ...
    <div id="vibrateButton" style="font-size: 48px; padding: 10px; background-color: transparent; color: #3b82f6; border: none; width: 60px; height: 60px; display: flex; align-items: center; justify-content: center; transition: color 0.2s, transform: 0.2s; position: absolute; top: 0; left: 30px; cursor: pointer; touch-action: none; z-index: 3;">💙</div>
    <div class="red-dot" style="width: 20px; height: 20px; background: radial-gradient(circle, red, #ff3333); border-radius: 50%; z-index: 3;"></div>
    <div class="pulse-symbol top" style="position: absolute; top: -30px; left: 50%; transform: translateX(-50%); font-size: 20px; color: #ff3333; z-index: 4;">〰️</div>
    <div class="pulse-symbol bottom" style="position: absolute; bottom: -30px; left: 50%; transform: translateX(-50%); font-size: 20px; color: #ff3333; z-index: 4;">〰️</div>
    <div class="glint top" style="position: absolute; top: 8px; left: 50%; width: 60px; height: 15px; background: radial-gradient(ellipse, rgba(255, 255, 255, 0.4), transparent); border-radius: 50%; transform: translateX(-50%) skewY(-10deg); z-index: 2;"></div>
    <div class="glint bottom" style="position: absolute; bottom: 8px; left: 50%; width: 60px; height: 15px; background: radial-gradient(ellipse, rgba(255, 255, 255, 0.4), transparent); border-radius: 50%; transform: translateX(-50%) skewY(10deg); z-index: 2;"></div>
    <div id="distortionLayer" style="position: absolute; left: 0; width: 120px; height: 60px; background-color: #1e1b4b; border-radius: 60px; z-index: 1;"></div>
  </div>
  <style>
    @keyframes pulse {
      0% { transform: scale(1); }
  ...
  <script>
    ...
    vibrateButton.addEventListener('mousedown', (e) => {
      e.preventDefault();
      isDragging = true;
      startY = e.clientY - vibrateButton.offsetTop;
    ...
    document.addEventListener('mousemove', (e) => {
      if (isDragging) {
        e.preventDefault();
        const trackRect = sliderTrack.getBoundingClientRect();
        let newX = e.clientX - trackRect.left - (vibrateButton.offsetWidth / 2) + 30;
        let newY = e.clientY - startY - trackRect.top;
        if (newX < 0) newX = 0;
    ...
    vibrateButton.addEventListener('touchstart', (e) => {
      e.preventDefault();
      isDragging = true;
      startY = e.touches[0].clientY - vibrateButton.offsetTop;
      const room = document.getElementById('room').value;
      if (room) {
        vibrateButton.style.backgroundColor = '#1e40af';
        vibrateButton.classList.add('pulsing');
    ...
    document.addEventListener('touchmove', (e) => {
      if (isDragging) {
        e.preventDefault();
        const trackRect = sliderTrack.getBoundingClientRect();
        let newX = e.touches[0].clientX - trackRect.left - (vibrateButton.offsetWidth / 2) + 30;
        let newY = e.touches[0].clientY - startY - trackRect.top;
        if (newX < 0) newX = 0;
        if (newX > trackRect.width - vibrateButton.offsetWidth) newX = trackRect.width - vibrateButton.offsetWidth;
        if (newY < 0) newY = 0;
        if (newY > trackRect.height - vibrateButton.offsetHeight) newY = trackRect.height - vibrateButton.offsetHeight;
