...
<body style="background: radial-gradient(circle at 50% 50%, rgba(20, 44, 102, 0.5) 10%, transparent 50%), radial-gradient(circle at 20% 30%, rgba(32, 16, 38, 0.5) 20%, transparent 50%), radial-gradient(circle at 80% 20%, rgba(14, 17, 36, 0.5) 25%, transparent 50%), radial-gradient(circle at 50% 80%, rgba(32, 16, 38, 0.5) 20%, transparent 50%), radial-gradient(circle at 30% 70%, rgba(14, 17, 36, 0.5) 20%, transparent 50%), linear-gradient(to bottom, #201026, #0e1124); color: white; font-family: Arial; margin: 0 auto; padding: 10px; height: 100vh; width: 100%; max-width: 414px; overflow: hidden; display: flex; flex-direction: column; align-items: center; justify-content: center; position: relative; box-sizing: border-box;">
  ...
  <div id="vibrateButton" style="background-color: transparent; border: none; display: flex; align-items: center; justify-content: center; transition: transform 0.2s; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); cursor: pointer; touch-action: none; z-index: 3;">
  ...
  <style>
    ...
    @keyframes gravityParticle {
      0% { opacity: 1; transform: translate(0, 0) scale(1); }
      100% { opacity: 0; transform: translate(var(--tx), 150px) scale(0.5); }
    }
    @keyframes pinch {
    ...
    .gravity-particle {
      position: absolute;
      pointer-events: none;
      animation: gravityParticle 2s cubic-bezier(0.4, 0, 0.2, 1) forwards;
    }
    .wave-burst {
      position: absolute;
      width: 100px;
  ...
  <script>
    ...
    // Create gravity particle
    function createGravityParticle() {
      const particle = document.createElement('div');
      particle.className = 'gravity-particle';
      particle.innerHTML = '<img src="/images/gravity-particle.png" alt="Gravity Particle" style="width: 12px; height: 12px;">';
      const trackRect = sliderTrack.getBoundingClientRect();
      const bodyRect = document.body.getBoundingClientRect();
      particle.style.left = (trackRect.left - bodyRect.left + trackRect.width / 2 - 6) + 'px';
      particle.style.top = (trackRect.top - bodyRect.top - 6) + 'px';
      const direction = Math.random() < 0.5 ? -1 : 1; // Random left or right arc
      const tx = direction * (30 + Math.random() * 30); // Horizontal offset 30-60px
    ...
    ws.onmessage = (event) => {
      ...
      if (data.room === document.getElementById('room').value) {
        if (data.command === 'startVibrate' && navigator.vibrate) {
          ...
          createGravityParticle(); // Spawn gravity particle on vibration
    ...
    vibrateButton.addEventListener('touchstart', (e) => {
      ...
      if (room) {
        ...
        const currentTime = Date.now();
        if (currentTime - lastHeartGelatinTime >= 500) {
          vibrateButton.classList.add('gelоко

System: I'm sorry, but it looks like your message was cut off. Based on the context and your previous requests, I'll assume you want to continue with the modifications to the web application described earlier. Below, I'll provide the updated `server.js` code with all the requested changes implemented:

1. **Remove the square behind the heart**: Eliminated the square hitbox around the `vibrateButton` by removing `padding`, `width`, and `height` styles, and setting `background-color: transparent`.
2. **Remove circles from toggleable buttons**: Removed `border-radius: 50%` from `.toggle-button` to make buttons non-circular, relying on the transparency of `pulse-toggle.png` and `wave-toggle.png`.
3. **Make toggle button PNGs 2x larger when toggled**: Added `transform: scale(2)` to `.toggle-button.toggled img` and increased the toggle button container size to `48px x 48px` to accommodate the larger images.
4. **Reduce amount of gravity particles and make them smaller**: Kept the number of gravity particles at one per vibration and reduced their size to `12px x 12px` in the `createGravityParticle` function.
5. **Improve the arc trajectory of gravity particles**: Updated the `gravityParticle` animation to use a `150px` vertical fall, `30-60px` horizontal offset, `2s` duration, and `cubic-bezier(0.4, 0, 0.2, 1)` for a smoother parabolic arc.

Since your message was cut off, I'll provide the complete updated code to ensure all changes are included and the application remains functional. The code below incorporates all the requested fixes while preserving the existing functionality (e.g., free heart movement, `sliderTrack` scaling, vibration, score system, WebSocket communication, etc.).

<xaiArtifact artifact_id="6019c051-f34e-4267-8326-e9e280e9207b" artifact_version_id="254d9077-86fc-4c23-9018-1ad5655f385d" title="server.js" contentType="text/javascript">
const express = require('express');
const WebSocket = require('ws');
const app = express();
const server = require('http').createServer(app);
